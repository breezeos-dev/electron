...
const ImmutableProperty =
  ...
  (target: T, propertyKey: keyof T) => {
    extendConstructHook(target, function (this: T) {
      (this as any)[hiddenProperties][propertyKey] = def(
        (this as any)._config,
        (k, v) => {
...
const LiveProperty =
  ...
  (target: T, propertyKey: keyof T) => {
    extendConstructHook(target, function (this: T) {
      ...
      if (onMutate)
        onMutate(this as any, (this as any)[hiddenProperties][propertyKey]);
    });
    Object.defineProperty(target, propertyKey, {
      get: function () {
        return this[hiddenProperties][propertyKey];
  ...
  implements Electron.TouchBarButton
{
  ...
  @LiveProperty<TouchBarButton>((config) =>
    typeof config.enabled !== "boolean" ? true : config.enabled
  )
  enabled!: boolean;

  @ImmutableProperty<TouchBarButton>(({ click: onClick }) =>
  ...
  implements Electron.TouchBarColorPicker
{
  ...
  @ImmutableProperty<TouchBarColorPicker>(
    ({ change: onChange }, setInternalProp) =>
...
class TouchBarGroup
  extends TouchBarItem<Electron.TouchBarGroupConstructorOptions>
  implements Electron.TouchBarGroup
{
  ...
  @LiveProperty<TouchBarGroup>(
    (config) =>
      config.items instanceof TouchBar
        ? config.items
        : new TouchBar(config.items),
...
class TouchBarLabel
  extends TouchBarItem<Electron.TouchBarLabelConstructorOptions>
  implements Electron.TouchBarLabel
{
  ...
  implements Electron.TouchBarSegmentedControl
{
  ...
  @LiveProperty<TouchBarSegmentedControl>((config) => config.backgroundColor)
  backgroundColor!: string;
  ...
  @ImmutableProperty<TouchBarSegmentedControl>(
    ({ change: onChange }, setInternalProp) =>
      typeof onChange === "function"
  ...
  implements Electron.TouchBarScrubber
{
  ...
  @LiveProperty<TouchBarScrubber>((config) =>
    typeof config.continuous === "undefined" ? true : config.continuous
  )
  continuous!: boolean;

  @ImmutableProperty<TouchBarScrubber>(
    ({ select: onSelect, highlight: onHighlight }) =>
      typeof onSelect === "function" || typeof onHighlight === "function"
        ? (
            details:
              | { type: "select"; selectedIndex: number }
              | { type: "highlight"; highlightedIndex: number }
          ) => {
...
class TouchBarOtherItemsProxy
  extends TouchBarItem<null>
  implements Electron.TouchBarOtherItemsProxy
{
...
class TouchBar extends EventEmitter implements Electron.TouchBar {
  ...
  static _setOnWindow(
    touchBar: TouchBar | Electron.TouchBarConstructorOptions["items"],
