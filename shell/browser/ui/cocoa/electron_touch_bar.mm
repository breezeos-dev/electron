...
- (void)updateSegmentedControl:(NSCustomTouchBarItem*)item
  ...
  std::string backgroundColor;
  if (settings.Get("backgroundColor", &backgroundColor) && !backgroundColor.empty()){
    control.wantsLayer = true;
    control.layer.backgroundColor = [self colorFromHexColorString:backgroundColor];
  } else {
    control.wantsLayer = false;
    control.layer.backgroundColor = nil;
  }

  CGFloat fontSize;
  control.font = [NSFont systemFontOfSize:fontSize];
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    ...
    [control setWidth:width forSegment:i];
